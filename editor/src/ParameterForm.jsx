...
function Field({ field, value, onChange, prefix = '', projectImages, onImageUpload, files, aiEnabled, onSettings }) {
  const id = `setting-${prefix}${field.name}`;
  const children = field.fields || [];
  if (field.type === 'group') {
    return <fieldset className="field-group"><legend>{field.label || field.name}</legend>{children.map(child => <Field key={child.name} field={child} value={value?.[child.name]} prefix={`${prefix}${field.name}-`} projectImages={projectImages} onImageUpload={onImageUpload} files={files} aiEnabled={aiEnabled} onSettings={onSettings} onChange={next => onChange({ ...(value || {}), [child.name]: next })} />)}</fieldset>;
  }
  ...
  return <div className={`field ${field.type === 'checkbox' ? 'field-switch' : ''}`}>
    ...
    {field.type === 'checkbox' ? <input {...shared} type="checkbox" className="toggle toggle-sm toggle-primary" checked={Boolean(value)} onChange={event => onChange(event.target.checked)} />
      : field.type === 'image' ? <ImageField id={id} field={field} value={value} onChange={onChange} projectImages={projectImages} onImageUpload={onImageUpload} files={files} aiEnabled={aiEnabled} onSettings={onSettings} />
...
export default function ParameterForm({ definition, values, onChange, projectImages = [], onImageUpload, files, aiEnabled, onSettings }) {
  return <div className="parameter-form">{definition.sections.map(section => <section className="parameter-section" key={section.id}><h3>{section.label}</h3>{section.fields.map(field => <Field key={field.name} field={field} value={values[field.name]} projectImages={projectImages} onImageUpload={onImageUpload} files={files} aiEnabled={aiEnabled} onSettings={onSettings} onChange={next => onChange({ ...values, [field.name]: next })} />)}</section>)}{!definition.fields?.length && !definition.sections.some(section => section.fields.length) && <p className="muted">Add @param declarations to build your form.</p>}</div>;
}
