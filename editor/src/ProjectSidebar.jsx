...
import { ArrowDownToLine, ChevronRight, FileCode2, FileImage, FilePlus2, Files, Folder, FolderOpen, FolderPlus, Pencil, Plus, ShieldCheck, Trash2, UploadCloud } from 'lucide-react';
...
export default function ProjectSidebar({ files, folders, active, onSelect, onCreate, onRename, onDelete, onMove, onUpload, onExport }) {
  ...
  return <aside className="file-sidebar">
    <div className="panel-heading"><span><Files size={15} /> PROJECT FILES</span><div className="create-menu"><button type="button" className="btn btn-ghost btn-xs btn-square" aria-label="Create file or folder" aria-expanded={menuOpen} onClick={() => setMenuOpen(open => !open)}><Plus size={15} /></button>{menuOpen && <div className="create-popover"><button type="button" onClick={() => { setMenuOpen(false); onCreate('file'); }}><FilePlus2 size={14} /> New file</button><button type="button" onClick={() => { setMenuOpen(false); onCreate('folder'); }}><FolderPlus size={14} /> New folder</button></div>}</div></div>
