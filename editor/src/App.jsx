...
import { ArrowDownToLine, ArrowUpFromLine, Check, ChevronDown, Code2, FileImage, HelpCircle, LoaderCircle, Monitor, PanelRightClose, PanelRightOpen, Plus, RotateCcw, Settings2, ShieldCheck, Smartphone, X } from 'lucide-react';
  ...
  return <div className="studio">
    ...
    <main className="workspace">
      ...
      <div className={`editor-shell ${previewVisible ? '' : 'preview-hidden'}`}>
        <ProjectSidebar files={files} folders={folders} active={active} onSelect={name => { setActive(name); if (!isTemplate(name)) setTab('code'); }} onCreate={kind => setDialog(kind === 'folder' ? 'add-folder' : 'add-file')} onRename={() => setDialog('rename')} onDelete={() => setDialog('remove')} onMove={moveEntry} onUpload={addUploadedFiles} onExport={() => exportZip(true)} />
