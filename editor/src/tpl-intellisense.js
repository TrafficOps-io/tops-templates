...
const DIALECT = 'safe-html-v1';
const registrations = new WeakMap();
const isTemplate = path => /\.tpl(?:\.html)?$/i.test(path);
 ...
 */
export function registerTplIntelliSense(monaco, { getProjectFiles }) {
  ...
  if (existing) { existing.update(getProjectFiles); return existing.disposable; }
  let readFiles = getProjectFiles, cache = new WeakMap();
  const load = (model, token) => {
    ...
    const entries = [[path, text], ...Object.entries(files)
      .filter(([name, value]) => name !== path && isTemplate(name) && typeof value === 'string')
      .sort(([a], [b]) => a.localeCompare(b))];
    const previous = cache.get(model);
    if (previous?.version === version && previous.uri === uri && previous.entries.length === entries.length
    ...
    const project = language.buildProject(documents, { dialect: DIALECT });
    if (cancelled(model, token, version)) return null;
    const snapshot = { version, uri, path, entries, project };
    cache.set(model, snapshot);
    return snapshot;
  };
  const providers = [];
  providers.push(monaco.languages.registerCompletionItemProvider(TPL_LANGUAGE_ID, {
    triggerCharacters: ['@', '{', '.', '&', ' ', ':', '(', '"', "'", '/', '[', ','],
  ...
  providers.push(monaco.languages.registerHoverProvider(TPL_LANGUAGE_ID, {
  ...
  providers.push(monaco.languages.registerSignatureHelpProvider(TPL_LANGUAGE_ID, {
  ...
  providers.push(monaco.languages.registerDefinitionProvider(TPL_LANGUAGE_ID, {
  ...
  registrations.set(monaco, { disposable, update(callback) { readFiles = callback; cache = new WeakMap(); } });
  return disposable;
}
