...
export default function PathInput({ value, onChange, folders, label, placeholder }) {
  ...
  return <div className="field path-field">
    ...
    {expanded && <div className="path-suggestions">
      <div className="path-suggestions-heading">{t("Existing folders")}</div>
      <div id={`${id}-options`} role="listbox" aria-label={t("Existing project folders")} className="path-suggestions-list">
    ...
    <p id={`${id}-help`} className="field-help">{expanded ? t("↑ ↓ to choose · Tab to complete · Esc to hide. ") : ''}{t("Choose an existing folder or type a new relative path.")}</p>
  </div>;
}
