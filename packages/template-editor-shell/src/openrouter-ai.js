...
- Declare values with @param name Type = default label="Label". Each option must occur only once per directive and have a value; do not combine = default with default= or repeat label=. Use required as a bare flag. Supported scalar types: String, Text, Wysiwyg, Markdown, Color, Number, Range, Boolean, Image, Url, Email, Select.
...
export async function projectFromAiResponse(payload, validateDraft) {
  ...
  await validateDraft({ files, mode: 'create' });
  return files;
}
...
export function valuesSchema(definition) {
  if (!definition || (!Array.isArray(definition.fields) && !Array.isArray(definition.sections))) throw new Error('Fix the template before generating its content.');
  return {
    type: 'object',
    properties: { values: objectSchema(definition.fields || definition.sections.flatMap(section => section.fields)) },
    required: ['values'],
    additionalProperties: false,
  };
}
...
function definitionForPrompt(definition) {
  ...
  return {
    ...
    fields: (definition.fields || definition.sections.flatMap(section => section.fields)).map(clean),
  };
}

export async function valuesFromAiResponse(payload, definition, files, validateDraft, signal) {
  ...
  await validateDraft({ files, values: payload.values, mode: 'edit', signal });
...
export async function generateTemplateWithOpenRouter(options) {
  ...
  return { ...result, files: await projectFromAiResponse(result.data, options.validateDraft) };
}
...
export async function fillTemplateWithOpenRouter(options) {
  ...
  return { ...result, values: await valuesFromAiResponse(result.data, definition, files, options.validateDraft, options.signal) };
}
